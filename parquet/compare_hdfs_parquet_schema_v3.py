...
def main(argv):
    ...
    if x == 0:
        ...
        first_parquet = get_hdfs_parquet(first).rstrip()
    else:
    ...
    if y == 0:
        ...
        second_parquet = second.rstrip()
    elif y == 1:
        print("Second Parquet in the given folder is: %s" % get_hdfs_parquet(second))
        second_parquet = get_hdfs_parquet(first).rstrip()
    else:
...
#    exit (0)
    ...
    print("=========================================================")
    print("Schema for first parquet: %s" %first)
    pprint.pprint(pDF1.dtypes)

    print("=========================================================")
    print("Schema for second parquet: %s" %second)
    pprint.pprint(pDF2.dtypes)
    ...
    print(str(len(dl1Notdl2)) + " columns in first parquet but not in second")
    pprint.pprint(dl1Notdl2)
    print("=========================================================")
    dl2Notdl1 = diff(dl2_fields, dl1_fields)
    print(str(len(dl2Notdl1)) + " columns in second parquet but not in first")
